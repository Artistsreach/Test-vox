...
const presentLessonFunctionDeclaration: FunctionDeclaration = {
  ...
  parameters: {
    type: Type.OBJECT,
    description: 'Presents an interactive HTML lesson to the student.',
    properties: {
      topic: {
        type: Type.STRING,
      ...
      htmlContent: {
        type: Type.STRING,
        description: 'The HTML code for the interactive lesson. Use Tailwind CSS for styling. It should be a self-contained HTML snippet (no html/body tags) suitable for rendering inside a div. Include interactive elements if possible.',
      },
    },
...
const VoxelWorld = forwardRef<VoxelWorldApi, VoxelWorldProps>(({
      ...
      if (state.instancedMesh) {
          ...
          if (intersections.length > 0) {
              ...
              if (intersection.instanceId !== undefined && intersection.face) {
                  ...
                  placePosition = [newPos.x, newPos.y, newPos.z];
              }
          }
      }
      if (!placePosition) {
  ...
  const handlePointerMove = (e: React.PointerEvent) => {
    ...
    if (state.touchState.activePointers.size === 1) {
        ...
        if (state.touchState.lastPinchDist !== null) {
            const pinchDelta = state.touchState.lastPinchDist - dist;
            if (isFreeCameraRef.current) {
                const ZOOM_SPEED = 0.05;
                state.freeCamera.zoom += pinchDelta * ZOOM_SPEED;
        ...
        const cx = (p1.x + p2.x) / 2;
        const cy = (p1.y + p2.y) / 2;
        if (state.touchState.lastTwoFingerCenter && isFreeCameraRef.current) {
             const panDx = cx - state.touchState.lastTwoFingerCenter.x;
             const panDy = cy - state.touchState.lastTwoFingerCenter.y;
  ...
  const updateWizard = (delta: number) => {
    ...
    if (state.wizard.isTalking && state.player.mesh) {
        const playerPos = state.player.mesh.position;
        const lookTarget = new THREE.Vector3(2 * state.wizard.mesh.position.x - playerPos.x, state.wizard.mesh.position.y, 2 * state.wizard.mesh.position.z - playerPos.z);
        state.wizard.mesh.lookAt(lookTarget);
    } else {
  ...
  const updateTeller = (delta: number) => {
    ...
    if (state.teller.isTalking && state.player.mesh) {
        const playerPos = state.player.mesh.position;
        const lookTarget = new THREE.Vector3(2 * state.teller.mesh.position.x - playerPos.x, state.teller.mesh.position.y, 2 * state.teller.mesh.position.z - playerPos.z);
        state.teller.mesh.lookAt(lookTarget);
    } else {
  ...
  const updateTeacher = (delta: number) => {
    ...
    if (state.teacher.isTalking && state.player.mesh) {
        const playerPos = state.player.mesh.position;
        const lookTarget = new THREE.Vector3(2 * state.teacher.mesh.position.x - playerPos.x, state.teacher.mesh.position.y, 2 * state.teacher.mesh.position.z - playerPos.z);
        state.teacher.mesh.lookAt(lookTarget);
    } else {
        ...
        if (worker.isPreparing) {
            ...
            if (worker.buildQueue.length > 0) {
                ...
                if (worker.buildCooldown <= 0) {
                    ...
                    if (voxelToBuild) onAddVoxel([voxelToBuild.position[0], voxelToBuild.position[1], voxelToBuild.position[2]], voxelToBuild.color);
             ...
             const playerPos = state.player.mesh.position;
             const lookTarget = new THREE.Vector3(2 * worker.mesh.position.x - playerPos.x, worker.mesh.position.y, 2 * worker.mesh.position.z - playerPos.z);
             worker.mesh.lookAt(lookTarget);
        }
    });
  };
  ...
  const updateCarCollisions = () => {
      ...
      const allCars = [
          ...state.cars.map(c => ({ type: 'npc' as const, data: c })),
          ...state.drivableCars.map(c => ({ type: 'drivable' as const, data: c }))
      ];
      ...
      for (let i = 0; i < allCars.length; i++) {
          for (let j = i + 1; j < allCars.length; j++) {
              ...
              const posA = carA.data.mesh.position;
              const posB = carB.data.mesh.position;
              ...
              if (dist < COLLISION_RADIUS * 2) {
                  ...
                  // Separate cars - Update both physics offset and visual mesh immediately
                  if (carA.type === 'npc') {
                  ...
                  // Elastic Bounce
                  const getVelocity = (c: typeof carA) => {
                      if (c.type === 'drivable') return c.data.velocity.clone();
                      const npc = c.data as any;
                      const fwd = new THREE.Vector3(0,0,1).applyQuaternion(npc.mesh.quaternion);
                      return fwd.multiplyScalar(npc.speed).add(npc.displacementVelocity);
                  };

                  const velA = getVelocity(carA);
                  const velB = getVelocity(carB);
                  ...
                  if (speed < 0) {
                      ...
                      const impulseMag = speed * -(1 + restitution) * 0.5;
                      const impulse = normal.clone().multiplyScalar(impulseMag);
                      
                      if (carA.type === 'drivable') {
                          carA.data.velocity.add(impulse);
                      } else {
                          const npc = carA.data as any;
                          npc.displacementVelocity.add(impulse);
                          // Impact absorbs some forward momentum
                          npc.speed *= 0.5;
                      }

                      const impulseB = impulse.clone().multiplyScalar(-1);
                      if (carB.type === 'drivable') {
                          carB.data.velocity.add(impulseB);
                      } else {
                           const npc = carB.data as any;
                           npc.displacementVelocity.add(impulseB);
                           npc.speed *= 0.5;
                      }
                  }
              }
    ...
    try {
        const response = await state.gemini.ai.models.generateContent({
            ...
            **Building Constraints & Guide:**
            ...
            4.  **Style**: Detailed, vibrant, and structurally sound. Use the full volume available if appropriate for the structure.
            
            Return ONLY the JSON array of voxels.`,
            config: {
                ...
                responseSchema: {
                    ...
                    items: {
                        type: Type.OBJECT,
                        properties: {
                            position: { type: Type.ARRAY, items: { type: Type.INTEGER }, description: "The [x, y, z] coordinates. X:-6 to 6, Z:-6 to 6, Y:0+" },
        ...
        if (Array.isArray(generatedVoxels)) {
            const worldVoxels = generatedVoxels
              .map((v: any) => ({ position: [v.position[0] + plotCenter.x, v.position[1] + plotCenter.y + 1, v.position[2] + plotCenter.z], color: v.color }))
  ...
  const startConversation = async (npc: NearbyNPC) => {
    ...
    try {
        ...
        else if (isWizard) config.tools = [{functionDeclarations: [spawnCustomNPCFunctionDeclaration, spawnObjectFunctionDeclaration]}];
                          ...
                          state.gemini.sessionPromise?.then((session) => { session.sendToolResponse({ functionResponses: [{ id : fc.id, name: fc.name, response: { result: result }, }] }) });
                            ...
                            const createProp = (type: string) => {
                                ...
                                if (lowerType.includes('sword') || lowerType.includes('blade') || lowerType.includes('katana')) {
                                    ...
                                    group.add(createVoxel(new THREE.Vector3(1, 0, 0), '#ffd700'));
                                    for(let y=1; y<=4; y++) group.add(createVoxel(new THREE.Vector3(0, y, 0), '#c0c0c0'));
                                    // Adjusted position to fit hand -4.0 * 0.18 = -0.72
                                    group.position.set(0, -4.0 * voxelSize, 0);
                                } else if (lowerType.includes('wing') || lowerType.includes('angel')) {
                                    const wingColor = '#ffffff';
                                    const createWing = (dir: 1 | -1) => {
                                        const w = new THREE.Group();
                                        w.add(createVoxel(new THREE.Vector3(0, 0, 0), wingColor));
                                        w.add(createVoxel(new THREE.Vector3(dir*1, 1, 0), wingColor));
                                        w.add(createVoxel(new THREE.Vector3(dir*2, 2, 0), wingColor));
                                        w.add(createVoxel(new THREE.Vector3(dir*3, 3, 0), wingColor));
                                        w.add(createVoxel(new THREE.Vector3(dir*1, 0, 0), wingColor));
                                        w.add(createVoxel(new THREE.Vector3(dir*2, 1, 0), wingColor));
                                        return w;
                                    };
                                    group.add(createWing(-1)); group.add(createWing(1));
                                    // Back is at Z=0.18. Move prop slightly out to 1.6*voxelSize ~ 0.288
                                    group.position.set(0, 2 * voxelSize, 1.6 * voxelSize);
                                    group.rotation.y = Math.PI; // Face backwards
                                } else if (lowerType.includes('wand') || lowerType.includes('staff')) {
                                    const len = lowerType.includes('staff') ? 8 : 4;
                                    for(let y=-1; y<len; y++) group.add(createVoxel(new THREE.Vector3(0, y, 0), '#5a3825'));
                                    group.add(createVoxel(new THREE.Vector3(0, len, 0), '#ff00ff'));
                                    // Adjusted position to fit hand
                                    group.position.set(0, -4.0 * voxelSize, 0);
                                } else if (lowerType.includes('shield')) {
                                    for(let x=-1; x<=1; x++) for(let y=-1; y<=1; y++) group.add(createVoxel(new THREE.Vector3(x, y, 0), '#c0c0c0'));
                                    group.position.set(0, -3.5 * voxelSize, 0);
                                } else if (lowerType.includes('cape') || lowerType.includes('cloak')) {
                                    for(let x=-1; x<=1; x++) for(let y=-4; y<=3; y++) group.add(createVoxel(new THREE.Vector3(x, y, 0), '#ef4444'));
                                    // Cape hangs from neck/shoulders. Z=1.6*voxelSize to sit on back.
                                    group.position.set(0, 2.5 * voxelSize, 1.6 * voxelSize);
                                }
                                return group;
                            };

                            if (args.heldProp) {
                                const prop = createProp(args.heldProp);
                                body_parts.armR.add(prop);
                            }
                            if (args.backProp) {
                                const prop = createProp(args.backProp);
